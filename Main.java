...
class Main {
  ...
  public static void main(String[] args) throws IOException, FileNotFoundException {
    ...
    while (line != null && lineCount < num_process) {
      ...
      if (lines.hasMoreTokens()) {
        for (int j = 0; j < num_resources; j++) {
          allocationMatrix[lineCount][j] = Integer.parseInt(lines.nextToken());
        }
      }
      line = br.readLine();
      lineCount++;
    ...
    // Initialize the number of solutions found to 0
    int numSolutions = 0;
    int[][] process_order_list = new int[2][num_process];
    ...
    do {
      ...
      // If all processes are finished, a solution has been found
      if (!unsafe && i == num_process) {
        numSolutions++;
        // Check if we have found the second solution
        if (numSolutions == 2) {
          break;
        }
        // Store the process order in the process_order_list
        for (int j = 0; j < num_process; j++) {
          process_order_list[numSolutions - 1][j] = process_order[j];
        }
        // Reset finish and process_order arrays for next solution
        for (int j = 0; j < num_process; j++) {
          finish[j] = false;
          process_order[j] = -1;
        }
        index = 0;
        // Reset available vector to initial state
        for (int j = 0; j < num_resources; j++) {
          current_available[j] = availableVector[j];
        }
      }
    ...
    if (numSolutions == 0) {
      ...
      // Print the process order for the single solution
      System.out.println("One solution found:");
      System.out.print("Process order of sequence: ");
      for (int i = 0; i < num_process; i++) {
        System.out.print((process_order_list[0][i] + 1) + " ");
      }
      System.out.println();
    } else {
      // Print the process orders for both solutions
      ...
      for (int i = 0; i < num_process; i++) {
        System.out.print((process_order_list[1][i] + 1) + " ");
      }
      System.out.println();
    }
